/**
 * Final Report Generator V2 - Using Professional Classifier
 * Structure:
...
function parseMessage(msg: MessageRecord): ParsedMessage {
  ...
  // Use professional classifier
  const classificationResult = classifyMessage(text);
...
export async function generateFinalReportV2(): Promise<{ url: string; key: string }> {
  const connection = await createConnection(process.env.DATABASE_URL || '');
  
  try {
    console.log('\n🔄 جاري إنشاء التقرير النهائي (المصنف المحترف)...\n');
    ...
    // Parse all messages with professional classifier
    const parsed: ParsedMessage[] = messages.map((m: MessageRecord) => parseMessage(m));
    ...
    const validDemandAreas = Object.entries(demandByArea).filter(([_, msgs]: [string, ParsedMessage[]]) => (msgs as ParsedMessage[]).length > 0);
    ...
    const key = `reports/MatchPro_Final_Professional_${timestamp}.xlsx`;
