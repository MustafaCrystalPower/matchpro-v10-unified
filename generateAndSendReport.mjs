import { generateAndUploadReports } from './server/newExcelReportGenerator.js';
import { sendReportEmail, generateReportEmailTemplate, verifyEmailConnection } from './server/_core/emailService.js';
import { generateReportMetrics } from './server/reportAnalytics.js';
...
async function main() {
  try {
    ...
    if (emailSent) {
      console.log(`[Report Generator] ✅ Report email sent successfully to ${process.env.REPORT_TO_EMAIL}`);
    } else {
