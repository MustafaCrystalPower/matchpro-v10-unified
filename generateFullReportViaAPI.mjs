...
try {
  ...
  const demands = demandsData.result?.data || [];

  // Fetch supplies
  const suppliesRes = await fetch(`${API_BASE}/supply.recent?input=%7B%7D`);
  const suppliesData = await suppliesRes.json();
  const supplies = suppliesData.result?.data || [];

  // Fetch matches
  const matchesRes = await fetch(`${API_BASE}/matches.recent?input=%7B%7D`);
  const matchesData = await matchesRes.json();
  const matches = matchesData.result?.data || [];
  ...
  const avgScore = matches.length > 0 ? (matches.reduce((a, m) => a + (m.matchScore || 0), 0) / matches.length).toFixed(2) : 'N/A';
  ...
  const summaryData = [
    ...
    { metric: 'High-Confidence Matches (≥75%)', value: highConfMatches.length },
    { metric: 'Hot Matches (≥90%)', value: hotMatches.length },
    { metric: 'Average Match Score', value: avgScore },
    { metric: 'Report Generated', value: new Date().toLocaleString('ar-EG') },
  ];
